...
files.forEach(filePath => {
    try {
        ...
        // Fix mobile menu icon - replace any mojibake
        const iconPatterns = [/в°|в™°|&#9776;|&#8801;/g];
        ...
        // Update navigation - check if it has old navigation structure
        if (content.includes('<nav class="main-nav">')) {
            // Check if navigation is old (no dropdown menu)
            const navSection = content.match(/<nav class="main-nav">[\s\S]*?<\/nav>/);
            if (navSection) {
                const navContent = navSection[0];
                // Old navigation doesn't have "ПРОДУКЦИЯ" dropdown
                const hasOldNav = !navContent.includes('nav-dropdown') || 
                                 (navContent.includes('СЕРТИФИКАТЫ') && !navContent.includes('ПРОДУКЦИЯ'));
